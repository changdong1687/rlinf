#! /bin/bash
# Launch the DreamZero LIBERO policy server with BWCache (block-level online cache).
#
# Usage:
...
#     --stats-out ./runs/bwcache/server_stats.json \
#     --device cuda:0 --port 8000
#
# Any extra args are forwarded to bwcache_server.py.
...
python "${EVAL_DIR}/bwcache_server.py" "$@"
